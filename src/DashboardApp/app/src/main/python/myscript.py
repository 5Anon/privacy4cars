...
def main():
    try:
        ...
        dest = r'/data/data/com.example.capstonepy/files/trackLog-2022-Jul-05_14-04-21.csv'
        data =  preprocessData(dest,'car31')
        if(not driving_privacy_setting):
            data = Anonymizer(parameters["Driving Behavior"], data) # TO-DO: NEED TO ADD CAR DATA TO ENABLE MORE QUASI-IDENTIFIERS
            meta_data = []
            for col in parameters["Driving Behavior"].keys():
                meta_data.append(data.columns.get_loc(col))
            ...
            return "Driving Behavior Anonymized"
        # elif(not location_privacy_setting):
            # # data = Anonymizer(parameters["Location Data"], data) # TO-DO: NEED TO ADD CAR DATA TO ENABLE MORE QUASI-IDENTIFIERS
            # # meta_data = []
            # # for col in parameters["Location Data"].keys():
            # #     meta_data.append(data.columns.get_loc(col))
            # # Meta_Data_col = [str(meta_data)]*len(data)
            # # data.insert(loc=1, column="Meta_Data",value=Meta_Data_col)
            # # uploadData = Upload('http://privacyforconnectedvehiclesapi.westus3.cloudapp.azure.com:3999/api/upload')
            # # uploadData.dataframe(data)
            # return "Location Data Anonymized"
        else:
            Meta_Data_col = [" "]*len(data)
            data.insert(loc=1, column="Meta_Data",value=Meta_Data_col)
            uploadData = Upload('http://privacyforconnectedvehiclesapi.westus3.cloudapp.azure.com:3999/api/upload')
            uploadData.dataframe(data)
            return "Not Anonymized"
    except Exception as e:
        return "Data Upload Failed: "+e
